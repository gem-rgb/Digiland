...
class CustomSignupForm(forms.Form):
    ...
    def clean_kra_pin(self):
        ...
        # NOTE: KRA database validation is disabled during signup.
        # Format validation is sufficient for account creation.
        # Database verification (GavaConnect / iTax) runs later as a
        # failover in production when KRA_DB_VALIDATION_ENABLED=True.
        logger.debug(f'KRA PIN format validation passed for {value[:3]}***')
        return value
